...
def create_new_chrome_instance(sender_id):
    ...
    user_data_dir = os.path.join(f"users", sender_id, f"chrome_profile")
...
def main():
    ...
    if broadcaster_id and sender_id:
        ...
        clean_screenshot_folder(username, output_folder)
        clean_screenshot_folder(username, description_folder)
        clean_screenshot_folder(username, comment_folder)
    else:
    ...
    take_screenshots_and_describe(driver, interval, run_duration, output_folder, description_folder, api_key, game_name, comment_folder, broadcaster_id, sender_id, client_id, access_token, stream_language, username)
