...
def get_disk_serial():
    ...
    try:
        if platform.system() == "Windows":
            ...
            serial = (
                subprocess.check_output("lsblk -o SERIAL", shell=True)
                .decode()
                .strip()
                .split("\n")[1]
                .strip()
            ...
            serial = (
                subprocess.check_output(
                    "system_profiler SPSerialATADataType | awk '/Serial Number/ { print $3 }'",
                    shell=True,
                )
                .decode()
                .strip()
...
def get_device_id():
    ...
    unique_device_id = "-".join(
        filter(lambda x: x and x != "unknown", device_id_components)
    )
