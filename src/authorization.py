...
def get_access_token(
    driver, user_id, client_id, client_secret, redirect_uri, username, password
):
    ...
    print_info(
        f"Token expired or not found for user {user_id}, requesting new authorization..."
    )

    authorization_code = get_authorization_code(
        driver, client_id, redirect_uri, "user:write:chat user:bot", username, password
    )

    if authorization_code:

        token_response = get_access_token_with_code(
...
def validate_token(access_token, client_id):
    ...
    if response.status_code == 200:
        ...
        print_error(
            f"Invalid token. Status code: {response.status_code}. Response: {response.text}"
        )
        return False
